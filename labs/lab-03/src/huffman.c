...
PriorityQueue* createPriorityQueue(unsigned int capacity) { 
  ...
  minHeap->front = (HuffmanNode**)malloc(minHeap->capacity * sizeof(HuffmanNode*)); 
  return minHeap; 
} 
...
void minHeapify(PriorityQueue* minHeap, int idx) { 
  ...
  if (left < minHeap->size 
      && minHeap->front[left]->char_freq 
              < minHeap->front[smallest]->char_freq) 
      smallest = left; 

  if (right < minHeap->size 
      && minHeap->front[right]->char_freq 
              < minHeap->front[smallest]->char_freq) 
      smallest = right; 

  if (smallest != idx) { 
      swapNodes(&minHeap->front[smallest], 
                      &minHeap->front[idx]); 
 ...
 */
void insertMinHeap(PriorityQueue* minHeap, 
...
{ 
    ...
    while (i 
           && HuffmanNode->char_freq 
                  < minHeap->front[(i - 1) / 2]->char_freq) { 
...
/**
 * Maintain the min-heap property of the PriorityQueue by percolating up each HuffmanNode
  *
  * @param pq - a pointer to the PriorityQueue
 */
 ...
 */
PriorityQueue* createPriorityQueueFromFreqArray(char data[], 
                                      int freq[], int size) 
...
HuffmanNode* buildHuffmanTree(char data[], 
    ...
    PriorityQueue* minHeap 
        = createPriorityQueueFromFreqArray(data, freq, size); 
